...
class Encoder(Module):
    ...
    def forward(self, x):
        # initialise an empty list to store the intermediate outputs
        blockOutputs = []

        # loop through the encoder blocks
        for block in self.encBlocks:
            # pass the inputs through the current encoder block, store the outputs, and
            # then apply maxpooling on the output
            x = block(x)
